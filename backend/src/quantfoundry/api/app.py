...
from quantfoundry.application.system.services import probe_health
...
from quantfoundry.infrastructure.db.idempotency import execute as execute_idempotent
...
_SCHEDULER_STATE_EVIDENCE_SECTION = "paper_scheduler_state_evidence.v1"
...
def _enforce_paper_scheduler_evidence_boundary(
    ...
    for audit in session.new:
        ...
        if not isinstance(summary, dict):
            continue
        state_evidence = summary.get(_SCHEDULER_STATE_EVIDENCE_SECTION)
        if state_evidence is None:
            continue
        if (
            set(summary) != {_SCHEDULER_STATE_EVIDENCE_SECTION}
            or not isinstance(state_evidence, dict)
            or set(state_evidence) != _SCHEDULER_STATE_EVIDENCE_KEYS
    ...
    *,
    ...
    s.add(
        Audit(
            ...
            sequence=next_sequence,
            request_id=effective_request_id,
            action=audit_action_type or canonical_event_type,
            object_type=kind,
            object_id=id,
            object_version=object_version,
            object_revision=effective_object_revision,
