...
`make lint`、`make typecheck`、`make governance`、`make hygiene`、`make secrets` 与 `make licenses` 是本地声明 gate。backend lint/typecheck 的 canonical cwd 固定为 `backend/`：Ruff 仅以 backend cwd 检查 backend 范围；mypy 必须以 `backend/` 为 cwd，并显式传入 `--explicit-package-bases app workers scheduler`，不得从 repository root 对 backend modules 作第二次解析。Python `==3.14.*` 是不降低的项目门槛；在 Python 3.13 上入口只能明确报告版本不兼容并非零退出，不能改用 3.13 解释器、锁文件或兼容回退伪造通过。
