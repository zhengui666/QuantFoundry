...
闭合 evidence 的每条记录必须绑定一个 immutable verification commit、独立 Test/Review 角色、GitHub Actions run identity 和一个可验证的、与该 run 直接绑定的 GitHub Actions artifact；verification commit 必须等于该 run 的 `head_sha`，最终 release/registry commit 必须是 verification commit 的 Git 祖先链后继。这样可以在验证完成后只追加 registry evidence/status；若追加了产品、契约、schema、迁移、workflow 或发布实现变更，必须重新验证，禁止复用旧 evidence。证据对象必须是 ZIP，内含声明路径的 JSON evidence report；registry 记录和内嵌 report 均须包含相同的 verification commit、run id、角色、canonical closure criteria 和已成功的验证命令。独立 Review/Test workflow 的 verification commit 仍是该 run 的 `head_sha`；当其通过输入验证已发布的历史 tag 时，release/supply-chain 命令必须读取并校验该 tag 的 immutable commit，不能把 workflow head 冒充为 release commit。为避免“上传后才知道 artifact URI、上传前却要求报告自带该 URI”的循环，内嵌 report 的 `artifact` 固定为 `{\"run_uri\":\"https://github.com/{repository}/actions/runs/{run_id}\"}`，其 attestation `subject_uri` 也固定为同一稳定 run URI；registry 的 `artifact_uri`/`artifact_sha256` 专门绑定该 GitHub Actions artifact，GitHub Release asset 只能作为长期归档的供应链/发布证据，不能作为 P0 closure evidence。registry `report.sha256` 与 attestation `subject_sha256` 绑定解包后 report 内容，report 不得内嵌自身 digest。Test report 的 content type 固定为 `application/vnd.quantfoundry.p0-test-evidence+json;version=1`，Review report 固定为 `application/vnd.quantfoundry.p0-review-evidence+json;version=1`；两类 evidence 必须来自不同的 GitHub Actions run。在线 `p0-check --require-closed`（online mode）必须以 `GITHUB_TOKEN`/`gh api` 解包并验证 report，验证 run 的 `head_sha`、verification commit 到最终 release commit 的祖先关系、artifact 的存在性与 SHA-256、report 全部身份字段和 attestation 元数据；网络、token、远端对象、ZIP/report、校验信息或 attestation 元数据缺失或不可验证时一律失败。`--require-closed` 在任何 release-blocking P0 未闭合时必须非零退出。`p0-check --offline-report`（兼容别名 `--report`）只作 registry schema/local report 生成，必须在 schema 有效时返回 0，即使存在未闭合 P0；它绝不执行远端 closure verification、不能用于 release、将 blocker 闭合或替代独立 evidence。GHCR digest 可作为 release supply-chain evidence，但不能单独充当可关闭 P0 的 evidence report。
...
兼容 shim 若仍被 Compose、worker 或 CLI 作为模块入口调用，必须保持同名模块的可执行入口语义；`python -m app.local_provider` 必须调用 canonical provider main，否则 `local-provider` Compose health gate 必须 fail-closed。
...
| `independent-agent-test` | 仅 `workflow_dispatch` | 对指定 commit 执行完整独立 Test Agent gate，并上传绑定 run 的结构化测试执行报告；release tag 输入存在时额外验证发布资产与供应链证据；失败也必须保留结果 artifact | `contents: read`、`actions: read` | 短期调试 artifact；不能单独关闭 P0 |
| `p0-release-close-gate` | 仅 `workflow_dispatch`，必须输入 release tag | 在全部 P0 已闭合后执行 online `p0-check --require-closed`，验证最终 registry 与 Release/GitHub Actions evidence | `contents: read`、`actions: read` | 短期调试 artifact；长期证据仍归档于 GitHub Release assets |
...
| `rc-release` | 仅 `v*` tag | 受限 evidence-bootstrap 的 `p0-check --require-closed-except-supply-chain` 或全闭合后的 `p0-check --require-closed`、完整 RC gate、Compose 实际镜像 GHCR publication、digest/SBOM/provenance/attestation/signature/checksum | `preflight`/`rc`: `contents: read`、`actions: read`；`publish`: `contents/packages/attestations/id-token: write` | GitHub Release assets（长期唯一证据） |
...
`rc-release` 仅接受远端存在且 checkout HEAD 精确等于 tag target 的 `v*` tag；正常发布在 P0 registry 未全部 `closed` 且完整 closure evidence 可验证前，`p0-check --require-closed` 必须先于 image build/publish 失败。唯一例外是上文定义的一次性 evidence-bootstrap transaction：它必须先通过 `p0-check --require-closed-except-supply-chain`，并将 supply-chain blocker 保持 `blocked`；随后必须由 Independent Test Agent 验证 release asset/GHCR/SBOM/provenance/signature 的可重放完整性，并由 Independent Review Agent 独立复核同一 release evidence；两者均须形成不同 run 的 closure evidence，最后在后继 registry commit 中闭合，不能形成 waiver。RC job 必须调用 `scripts/ci/run-gate.sh rc` 这一唯一完整入口；该入口按顺序执行 online P0 验证、known-issue review、full CI、fresh Compose/PG18 migration、backup/restore，并将每项命令、退出码和环境限制写入结构化结果。无 tag、GitHub run identity、镜像 digest、SBOM/provenance/attestation 或签名/校验清单时，任何 release manifest、GitHub Release 或成功状态均不得创建。仅具有 `contents: write` 的 publish job 可以创建 draft Release：它必须 create-or-validate 同 tag/ref/commit 的 draft，复用正确绑定的既有 draft，已发布一律 fail-closed。GitHub UI 通过“Existing tag”创建的 draft 可能将 `target_commitish` 保存为 `main` 等分支值；此时只要远端 tag/ref 已由 preflight 精确解析到同一 immutable commit，即视为兼容绑定，不能仅因该 UI 元数据值不同而阻断发布。随后上传所有唯一 inventory assets；上传完成后必须从远端重新读取 asset inventory、下载 `SHA256SUMS` 和 manifest 并逐项复核名称、完整性、manifest binding 及 SHA-256。仅所有远端复核成功后可 publish draft；任何构建、上传或复核失败都必须保留 draft，且不得公开半成品。发布后必须将同一 tag/ref/commit 绑定的 manifest、P0 registry、known-issue registry、gate reports、Compose digest binding、SBOM/provenance/attestation/signature verification 与 `SHA256SUMS` 作为 Release assets 上传。release manifest 的 `checksums` 字段和 `release_assets` 必须明确包含 `release-manifest.json` 与 `SHA256SUMS`，并以发布时的唯一上传名而非本地 basename 为准。
