...
if ! QF_RELEASE_GOVERNANCE_ROOT="$repo_root" \
  "$repo_root/scripts/ci/release-governance-static-gate.sh" --skip-fixtures >"$fixture_root/real-config.out" 2>&1; then
...
PY
  ...
  if QF_RELEASE_GOVERNANCE_ROOT="$fixture_root" \
