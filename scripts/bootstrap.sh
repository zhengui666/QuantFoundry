...
openai_base_url="$(environment_value QF_OPENAI_BASE_URL)"
openai_models="$(environment_value QF_OPENAI_MODELS)"
if [[ "$openai_base_url" == "http://local-provider:8011/v1" ]]; then
  ...
  [[ "$provider" == "openai-compatible" && "$openai_models" == *"qf-local-v1"* ]] || {
    printf '%s\n' 'The bundled local provider requires openai-compatible/qf-local-v1.' >&2
    exit 1
  }
  ...
  printf '%s\n' 'Local bootstrap requires QF_OPENAI_BASE_URL=http://local-provider:8011/v1.' >&2
  exit 1
fi
